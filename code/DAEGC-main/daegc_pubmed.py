...
def trainer(x, y, adj_label, adj):
    ...
    for epoch in range(args.max_epoch):
        ...
        if epoch % args.update_interval == 0:
            ...
            # update_interval
            A_pred, z, Q = model_cpu(data, adj, M)
        ...
        for batch in split_list:
            ...
            A_pred, z, q = model(data_batch, adj_batch, M_batch)
            p = target_distribution(Q[batch].detach())

            kl_loss = F.kl_div(q.log(), p.to(device), reduction='batchmean')
            # TODO 如果adj_label不是稀疏矩阵，就：adj_label[batch, :][:, batch].contiguous().view(-1)
            re_loss = F.binary_cross_entropy(A_pred.view(-1).cpu(), adj_label.to_dense()[batch, :][:, batch].contiguous().view(-1))
