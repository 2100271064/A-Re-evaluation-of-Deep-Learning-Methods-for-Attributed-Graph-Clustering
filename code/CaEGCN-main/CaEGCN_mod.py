...
def train_sdcn(dataset, n_outliers):
    ...
    for epoch in range(400):
        ...
        #     print("当前epoch={}, kl_loss=inf".format(epoch))
        #     # 将log后inf修正为0
        #     q_log = q.log()
        #     zero_location = torch.where(torch.isinf(q_log))
        #     for index in range(len(zero_location[0])):
        #         row, cul = zero_location[0][index], zero_location[1][index]
        #         q_log[row, cul] = 0
        #     kl_loss = F.kl_div(q_log, p, reduction='batchmean')
        print("当前epoch={}, kl_loss={}".format(epoch, kl_loss))
        if args.name == "wiki":
            ce_loss = F.kl_div(pred, p, reduction='batchmean')  # 与概率p比？
            ce_loss *= 0.01
        else:
            ce_loss = F.kl_div(pred.log(), p, reduction='batchmean')  # 与概率p比？
        # ce_loss = F.kl_div(pred.log(), p, reduction='batchmean')  # 与概率p比？
        # if torch.isinf(ce_loss):
        #     print("当前epoch={}, ce_loss=inf".format(epoch))
        #     # 将log后inf修正为0
        #     pred_log = pred.log()
        #     zero_location = torch.where(torch.isinf(pred_log))
        #     for index in range(len(zero_location[0])):
        #         row, cul = zero_location[0][index], zero_location[1][index]
        #         pred_log[row, cul] = 0
        #     ce_loss = F.kl_div(pred_log, p, reduction='batchmean')
        #     ce_loss *= 0.01
        #     print("修改后，ce_loss={}".format(ce_loss))
        ...
        # print("A_pred", A_pred, torch.max(A_pred), torch.min(A_pred))
        # tmp = adj.to_dense().view(-1)
        # print("adj.to_dense().view(-1)", tmp, tmp.max(), tmp.min())
        # A_pred[A_pred > 1] = 1
        # A_pred[A_pred < 0] = 0
        ...
        print("当前epoch={}, loss={}".format(epoch, loss))
