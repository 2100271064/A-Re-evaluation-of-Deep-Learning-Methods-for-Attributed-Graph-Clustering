...
def train(dataset, args):
    ...
    model = DGCEFR(num_features=args.input_dim, hidden1_size=args.hidden1_dim, hidden2_size=args.hidden2_dim,
                  embedding_size=args.hidden3_dim, alpha=args.alpha, num_clusters=args.n_clusters, n_z=args.n_z).to(args.device)
    print(model)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.update_interval == 0:
            ...
            # update_interval
            A_pred, z, tmp_q, x_bar = model_cpu(data, adj, M)
            tmp_q = tmp_q.data
        ...
        for batch in split_list:
            ...
            A_pred, z, q, x_bar = model(data_batch, adj_batch, M_batch)

            p = p.to(args.device)
            kl_loss = F.kl_div(q.log(), p[batch], reduction='batchmean')
            re_loss_gae = F.binary_cross_entropy(A_pred.view(-1), adj_label.to_dense()[batch, :][:, batch].view(-1))
            re_loss_ae = F.mse_loss(x_bar, data_batch)
...
if __name__ == "__main__":
    ...
    print('final_acc: {:.4f}, fianl_var_acc: {:.2f}, final_std_acc:{:.2f}'.format(value, var, std))

    value = np.mean(nmi_arr)
    var = np.var(nmi_arr)
    std = np.std(nmi_arr)
    print('final_nmi: {}, final_var_nmi: {}, final_std_nmi:{}'.format(value, var, std))
    print('final_nmi: {:.4f}, final_var_nmi: {:.2f}, final_std_nmi:{:.2f}'.format(value, var, std))

    value = np.mean(ari_arr)
    var = np.var(ari_arr)
    std = np.std(ari_arr)
    print('final_ari: {}, final_var_ari: {}, final_std_ari:{}'.format(value, var, std))
    print('final_ari: {:.4f}, final_var_ari: {:.2f}, final_std_ari:{:.2f}'.format(value, var, std))

    value = np.mean(f1_arr)
    var = np.var(f1_arr)
    std = np.std(f1_arr)
    print('final_f1: {}, final_var_f1: {}, final_std_f1:{}'.format(value, var, std))
    print('final_f1: {:.4f}, final_var_f1: {:.2f}, final_std_f1:{:.2f}'.format(value, var, std))
