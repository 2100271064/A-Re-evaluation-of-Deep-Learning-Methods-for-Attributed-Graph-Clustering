...
def train(model, data, original_adj, adj, adj_label, label, n_outliers, device):
    ...
    with torch.no_grad():
        # ATT_AE
        # _, _, z, _, _, _ = model(data, adj)
        # GCN
        # _, _, z, _, _ = model(data, adj)
        # print("GCN")
        # 组合模型3
        # _, _, _, _, _, _, _, _, z, _, _ = model(data, adj)
        # 组合模型4/5
        _, _, _, _, z, _, _ = model(data, adj)
        # print("组合模型4/5")
        # Combine2
    ...
    for epoch in range(opt.args.epoch):
        ...
        # length = len(pos_inds)
        # sampled_neg = torch.LongTensor(np.random.choice(neg_inds, size=length))
        # neg_inds_device = torch.LongTensor(sampled_neg).to(device)
        # sampled_inds = torch.cat((pos_inds_device, neg_inds_device), 0)
        ...
        # sample_label = torch.cat((torch.ones(len(pos_inds)), torch.zeros(len(sampled_neg)))).to(device)
        ...
        if (epoch + 1) % opt.args.upd == 0:
            model.eval()
            # update_interval
            # _, _, _, _, z, q_h, q_z, _ = model(data, adj)
            # p_h = target_distribution(q_h.data)
            # p_z = target_distribution(q_z.data)
            # # _, _, z, _, q, _ = model(data, adj)
        ...
        # 基础模型
        # GCN
        # z_hat, adj_hat, z, z_cluster, q = model(data, adj)
        # GAT
        # z_hat, adj_hat, z, _, _, _ = model(data, adj)
        # 组合模型3
        # h_hat, adj_hat_ae, z_gcn_hat, adj_hat_gcn, z_hat, adj_hat, z_ae, z_gcn, z, q_h, q_z = model(data, adj)
        # h_hat, adj_hat_ae, z_gat_hat, adj_hat_gat, _, adj_hat, z_ae, z_gat, z, q_h, q_z = model(data, adj)
        # 组合模型4/5
        h_hat, adj_hat_ae, z_hat, adj_hat, z, q_h, q_z = model(data, adj)
        # Combine2
        ...
        # loss_Z2X = F.mse_loss(z_hat, data)
        # loss_H2X = F.mse_loss(h_hat, data)
        # loss_Z2A = F.binary_cross_entropy(adj_hat.view(-1), adj_label.to_dense().view(-1))
        # loss_Z2AX = F.mse_loss(z_hat, torch.spmm(adj, data))
